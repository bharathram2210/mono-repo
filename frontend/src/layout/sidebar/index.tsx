...
const Sidebar: React.FC<HeaderProps> = ({ open, handleDrawerClose, drawerWidth }) => {
                ...
                {adminSidebarItems.map((item, index) => {
                    return (
                        <Tooltip key={index} title={item.name} placement='right' arrow>
                            <ListItem onClick={() => navigateTo(item.path)} key={index} disablePadding sx={{ display: 'block' }}>
                                <ListItemButton
                                ...
                                >
                                    ...
                                    >
                                        <item.icon fontSize='large' color={pathname === item.path ? 'primary' : 'secondary'} />
                                    </ListItemIcon>
                                    <ListItemText primary={item.name} sx={{ opacity: open ? 1 : 0, color: 'grey', fontWeight: '900' }} />
                                </ListItemButton>
                            </ListItem>
                        </Tooltip>
                    );
